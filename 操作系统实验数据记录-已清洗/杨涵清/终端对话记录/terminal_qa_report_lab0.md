...
收标志                                                                         │  ├──────────────────────────────────────────────┼────────────────┼─────────────
──────────────────────────────────────────────────────────────────────────┤
  │ 任务 1~3：环境准备（虚拟机 + 工具链 + QEMU） │ 无（装好就行） │ riscv64-unknown-elf-gcc                                                                    -
-version                                                                       qemu-system-riscv64                                                             -
-version                                                                       有
输出，QEMU                                                                     7.0                                                                             │
  ├──────────────────────────────────────────────┼────────────────┼───────────────────────────────────────────────────────────────────────────────────────┤
  │ 任务 4：编译内核                             │ make build     │ 生成 target/kernel                                                                         │
  ├──────────────────────────────────────────────┼────────────────┼───────────────────────────────────────────────────────────────────────────────────────┤
  │ 任务 5：做文件系统镜像                       │ make fs        │ 生成 fs.img（
要                                                                             sudo                                                                            密
码）                                                                           │  ├──────────────────────────────────────────────┼────────────────┼─────────────
──────────────────────────────────────────────────────────────────────────┤
  │ 任务 6~7：启动并验证                         │ make run       │ 依次看到 OpenSBI                                                                           b
anner                                                                          内
核                                                                             banner                                                                          -
>                                                                              $，然后                                                                         l
s、cat                                                                         README                                                                          有
...
──────────────────────────────────────────────────────────────────────────┘
  ...
  但要注意：任务从哪一步开始，完全取决于你现在的环境——实验镜像里工具链和 QEMU 都预装好了，任务                                                                 2
、3                                                                            可
以直接跳过；基础镜像要自己装；自己电脑上全新装的系统则连环境手册的任务         都
得走。
  ...
  （如果不确定：在终端里跑一下 riscv64-unknown-elf-gcc --version，把结果告诉我。）
...
● 差一步——你发给我的是命令本身，但我需要的是它运行后的输出。
  ...
  riscv64-unknown-elf-gcc --version 这行字发给我，只是让我看到你打了什么，我并不  能因此知道你的机器上装没装这个工具链。

  两种办法，选一个：

  1. 在输入框里加个感叹号：输入 ! riscv64-unknown-elf-gcc --version（开头的 !
  别漏）。! 前缀会让命令直接在本环境执行，输出会自动落到我们的对话里，我能看到。  2. 自己开个终端跑：在任何终端窗口里执行这条命令，然后把输出复制粘贴给我。
...
● 这个报错本身值得停一下——它暴露了一个小概念：
  ...
  虚拟机里——等会儿确认 QEMU 版本时，顺便告诉我这台虚拟机的镜像是课程发的实验镜像  还是基础镜像，这决定你后面要不要补装东西。现在先把 QEMU 版本跑出来。
...
● 8.2.2，过了（要求 ≥ 7.0）。两关都确认完，工具链和 QEMU
  ...
  只提前打一个招呼：8.2.x 内置的是 OpenSBI 1.3，正是 FAQ 里“看到 -> / $ 但键盘不  回显”那条的组合。本仓库已经带了兼容修复，正常情况不会遇到；真遇到了你翻 FAQ
  那一行就知道是怎么回事，不用慌。
...
● 这条输出不是报错，但它值得停下来讲一个 make 的核心机制——你以后每次敲 make
  ...
  build 自己一行命令都没有，只是两个依赖的别名。我刚才查了你的目录，事实是：targ  et/kernel（250K）、fs.img（256M）、xv6-user/_init、_sh
  ...
  1. 验收点说的是“make build 无报错”——你现在还没见过它真正编译，万一预构建的产物  和你现在的工具链对不上，问题会被藏到后面；
  ...
  跑完还是老规矩：贴最后十几行，我看有没有 Error、有没有重新生成 target/kernel。✻ Worked for 1m 57s
...
| make ru | 1 |
| mkfs.vfat --help | 1 |
| riscv64-unknown-elf-gcc --vers | 1 |
| sudo chown ailab-os:ailab-os f | 1 |
| sudo make fs | 1 |

...
● 看完了，我用大白话给你捋一遍 lab0 是干什么的。
  ...
  跑通"的章。你要做的不是写代码，而是理解"为什么我的电脑上要装这些东西、每一步在  干嘛"。
  ...
  这正好接回我上一条问你的问题——你现在用的到底是课程镜像，还是自己全新装的系统？  这决定了你要不要做安装那两步。
...
● 好，既然是学校提供的镜像，那大概率环境里已经装好东西了，我们先别急着装任何东西  。
  ...
  第一步很简单：花一分钟，确认这台机器上到底已经有什么。请你在自己的终端里跑这两  条命令，然后把输出贴给我：
...
● 这条是命令本身——我要看的是它跑完之后屏幕打印出来的那几行字。
  ...
  请把你屏幕上实际显示的那几行复制粘贴给我。如果敲完之后没反应、或者出现 command  not found 之类的报错，也原样贴给我。
...
● 好，目录没问题。
  ...
  敲完回车，屏幕上会刷出一大堆编译信息。不用逐行看，你只要把最后几行（尤其是有没  有 Error 或者 make: *** 这种字样）贴给我就行。
...
● 好问题，这其实戳中了一个隐藏概念——"终端"是什么。
  ...
  本身就运行在终端里。终端就是那个"你输入命令、电脑去执行"的黑窗口。所以你不是没  有终端——你已经在终端里了，只是你现在正用它跟我对话。
...
● 成功了，而且这个输出其实是个好消息。
  ...
  先跑，把结果（尤其最后几行）发我。如果它刷出一堆文字后停住不动了，也是正常的——  那可能正是我们要找的东西。
...
● 先说最重要的事：你的 xv6 已经成功跑起来了！
  ...
  -> / $ 就是我们要的 shell 提示符。 它出现了，说明 lab0 的核心目标——让 xv6-k210  在 QEMU 里启动到 shell——已经达成。前面那些 OpenSBI

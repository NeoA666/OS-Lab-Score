...
Remote GNOME desktop is **already running** on display `:5` at `admin` (`59.64.94.133`).
You only need to do the local Windows steps in "How To Start" below.

## Verified Environment

- Server: `admin` (login node), `CentOS Linux release 8.10 (Green Obsidian)`, user `zhaoxia`.
...
Step 2 — open your VNC viewer and connect to:

```text
127.0.0.1:5905
```

Step 3 — enter the existing VNC password locally in the viewer when prompted.
The password is the one already stored in `~/.vnc/passwd`; it was not changed.
...
Run `vncpasswd` interactively on the server. This **replaces** the existing VNC
password, so confirm first. Never send the password through chat or a script.

        ...
        while True:
            ...
            if target.exists():
                ...
                available = available and target.is_dir() and (
                    (isinstance(owner, dict) and owner.get("source") == info["source"])
                    or (owner is None and overwrite))
            if available:
                break
            sequence += 1
...
def write_student_reports(info, files, commands, full_sessions, claude_sessions, errors,
    ...
    full = header("Full Terminal Transcript") + [f"- 终端录像总数：{len(files)}",
        f"- 成功重放数量：{sum('error' not in s for s in full_sessions)}",
        f"- 重放失败数量：{sum('error' in s for s in full_sessions)}", "",
        "由 pyte 按 timing 逐帧重放，保留清屏历史及变化行，不执行 Claude 清洗。",
        "Frame 内的数字是 Screen/滚动历史行号；空内容表示该行被清空。变化行需要结合前帧阅读。", ""]
    for s in full_sessions:
        full.extend([f"## Session {s['rec']}", ""])
        if "error" in s:
        ...
        for frame, elapsed, lines in s["frames"]:
            full.extend([f"### Frame {frame + 1} · +{elapsed:.3f}s", ""])
            append_fenced_text(full, [f"{row}: {line}" for row, line in lines])
            full.append("")
    ...
    for number, s in enumerate(claude_sessions, 1):
        ...
        for turn, pair in enumerate(s["pairs"], 1):
            qa.extend([f"### Turn {turn}", "", "#### User", "", pair["user"], "",
                       "#### Claude", "", pair["claude"], ""])
    # 首次写入前登记归属；下次只允许本来源自动更新这四个固定报告。
    atomic_write(info["output"] / OWNER_FILE, json.dumps(
        {"tool": "replay_term_qa", "source": info["source"], "reports": REPORT_NAMES},
        ensure_ascii=False, indent=2))
    for name, content in zip(REPORT_NAMES, (report, full, stats, qa)):
        atomic_write(info["output"] / name, "\n".join(content))
    return {"recordings": len(files), "commands": len(commands), "claude_sessions": len(claude_sessions),
            "turns": turns, "replay_failed": sum("error" in s for s in full_sessions)}


def process_student(info):
    """每个录像的三条提取路径独立容错，某一路失败不丢弃其他已得到的结果。"""
    ...
    for number, out in enumerate(files, 1):
        ...
        if not read_timing_entries(tim):
            message = f"{tim}：计时文件缺失、为空或不可读，使用一次性重放降级"
            errors.append(message)
            print(f"  [警告] {message}", flush=True)
        start = ""
        for stage in ("完整重放", "Shell 提取", "Claude QA 提取"):
            try:
                if stage == "完整重放":
                    session = replay_full_terminal(out, tim)
                    full.append(session)
                    start = session["start"]
                elif stage == "Shell 提取":
                    _, start, extracted = extract_session(str(out), str(tim))
                    commands.extend(extracted)
                    shell_success += 1
                else:
                    _, pairs = extract_claude_session(str(out), str(tim))
                    claude.append({"rec": rec, "start": start, "pairs": pairs})
            except Exception as exc:
                message = f"{out} [{stage}] {type(exc).__name__}: {exc}"
                errors.append(message)
                print(f"  [失败] {message}", flush=True)
                if stage == "完整重放":
                    full.append({"rec": rec, "file": str(out), "error": str(exc)})
    result = write_student_reports(info, files, commands, full, claude, errors, shell_success)
    result.update(status="部分失败" if any(" [" in e for e in errors) else "成功", info=info, errors=errors)
    return result


def write_readme(output_dir, input_dir, results, skipped):
    """生成中文使用说明和本次运行的可核对汇总（筛选运行只汇总本次）。"""
    md = [README_TITLE, "", "## 工具用途与数据来源", "",
          ...
          *[f"    {'└──' if i == 3 else '├──'} {name}" for i, name in enumerate(REPORT_NAMES)], "```", "",
          "| 文件 | 内容 |", "| --- | --- |",
          "| terminal_qa_report.md | Shell 完整命令、执行目录、输出、录像编号、开始及相对时间、命令频次；每条输出最多 400 行 |",
          "| full_terminal_transcript.md | 所有录像的 pyte 逐帧变化行、清屏历史、TUI、spinner、状态栏和重放错误 |",
          "| command_statistics.md | 仅 Shell 命令，按次数降序、命令文本升序排列 |",
          "| claude_qa_clean.md | 有效 Claude Session、编号 Turn 及用户问题与最终回答，含会话和全局统计 |", "",
          "`terminal_qa_report.md` 按命令组织输出，可能截断长输出；`full_terminal_transcript.md` 按录像和帧组织，"
          "保留实际终端界面变化，不执行 QA 清洗。Frame 中行号指向 Screen/滚动历史行；空白变化也保留。",
          ...
          "隐藏文件 `.replay_term_qa.json` 记录输出归属。默认可原子替换本工具为相同来源生成的四份报告。"
          "无归属的既有目录默认另选名称；`--overwrite` 可允许覆盖此类目录中的四个固定文件，仍禁止覆盖已标记为其他来源的结果。"
          "不清理旧文件或原始数据。每个文件单独原子替换，整个学生目录不是跨文件事务。",
          ...
          "- 完整转写保留 timing 帧及清屏前状态；同一 timing 数据块内部被原地覆盖的瞬时字符未必可恢复。",
    ...
    for r in results:
        info = r["info"]
        link = quote(info["output"].name) + "/terminal_qa_report.md"
        output = f"[查看报告]({link})" if "recordings" in r else "—"
...
def main(argv=None):
    """批处理入口；仅输入/输出配置错误阻止启动，学生内部异常逐个隔离。"""
    parser = argparse.ArgumentParser(description="批量重放学生 Linux script 录像，生成四份 Markdown 报告")
    ...
    parser.add_argument("--overwrite", action="store_true", help="允许覆盖无归属输出目录中的固定报告")
    ...
    try:
        ...
        return 1
    print(f"汇总：{len(results)} 个学生，{sum(r.get('recordings', 0) for r in results)} 个录像，"
    ...
    return int(any(r["status"] in ("失败", "部分失败") for r in results))

...
README_UPDATE_NOTES = [
    ...
    "- Claude 问答按启动所在目录归类；返回同一 lab 后，同一录像的问答合并为一个 Session，Turn 连续编号。完整转写保留片段编号、原始字节区间和累计时间。",
...
def write_readme(output_dir, input_dir, results, skipped):
    ...
    md = [README_TITLE, "", *README_UPDATE_NOTES, "## 工具用途与数据来源", "",
          ...
          "| full_terminal_transcript_labN.md / full_terminal_transcript_other.md | 所有录像的 pyte 逐帧变化行、清屏历史、TUI、spinner、状态栏和重放错误 |",
          ...
          "`terminal_qa_report_labN.md` 按命令组织输出，可能截断长输出；`full_terminal_transcript_labN.md` 按录像和帧组织，"

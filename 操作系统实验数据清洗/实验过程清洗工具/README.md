...
- 重放 `term/*.out.gz` 和对应的 `*.tim.gz`，处理 ANSI 控制序列、退格、清屏和自动换行。
- 生成四类按实验分类的报告：终端对话记录、完整终端转写记录、终端命令统计和 Claude 对话。
- 根据提示符中的工作目录识别实验分类；无法识别目录、非 `lab0` 至 `lab8` 的记录和读取失败的录像归入 `other`。
...
批处理输入根目录通常为上级的 `操作系统实验数据记录/`。每个学生目录名称应符合以下形式：
...
- `term/*.out.gz` 是必需的终端录像数据。
- 同名 `*.tim.gz` 为可选计时文件。缺少或不可读时会降级为一次性重放，时间线和问答可能不完整。
...
`--overwrite` 仅允许覆盖没有本工具归属信息的输出目录中的固定程序产物；它不会删除其他文件，也不会覆盖已归属给其他原始提交的结果。
...
主工具会针对每名学生分别计算输入指纹。修改或新增录像会重建对应学生的报告和时间线；仅修改 `logs/events.jsonl` 时只会重建时间线。缺少 `term/`、损坏录像或单个时间线处理错误不会中断其他学生，根目录 `README.md` 会列出本次跳过和部分失败的原因。存在学生或录像失败时命令退出码为 `1`。

输出文件通过临时文件原子替换，工具会拒绝向符号链接写入。清理过时产物时仅处理归属清单中登记的本工具文件，原始数据及其他文件保持不变。

...
| `代码差异报告工具/` | xv6 `lab0` 至 `lab8` 源码差异 Markdown 生成器及其测试 | 否 |
...
三个工具直接将同一份结果写入两套完整实体文件，默认根目录为本目录下的 `操作系统实验数据记录-已清洗/`，不需要运行后整理或复制。
...
按 Lab 分类预建 lab0–lab8 和“其他”；学生与工具目录按实际结果创建。学生名称冲突沿用身份消歧规则。工具内部只从“按人分类”发现输入，避免两份结果重复分析。隐藏清单记录来源与缓存，不属于阅读报告。

实验过程工具每个 Lab 的文件名为 `终端对话记录.md`、`完整终端转写记录.md`、`终端命令统计.md`、`Claude对话记录.md`、`实验过程时间线.md`、`简洁实验过程时间线.md` 和 `实验过程时间线.json`。仅汉化文件名，正文和 JSON 字段保持原有语义。
...
个人报告写入两种分类各自的 `代码差异报告工具/代码差异报告.md`；跨学生汇总统一写入 `汇总报告/代码差异报告汇总/代码差异报告汇总-labN.md`，日志写入 `运行日志/`。详细参数见 [代码差异工具说明](代码差异报告工具/README.md)。
...
实际 NIM 分析与独立复核都要求由运行环境提供 `NVIDIA_API_KEY`。输出直接写入两种分类的 `AI人工贡献识别工具/` 目录，详细命令、v3 契约与诚信系统消费限制见 [AI/人工贡献识别工具说明](AI人工贡献识别工具/README.md)。

本次目录改造只对 AI 工具做离线格式测试，不调用真实 API。旧结果由工具的 `migrate-legacy` 入口原样迁入双分类，并附历史说明；旧证据路径与哈希保留，不冒充当前输入的有效分析缓存。模拟测试只写临时目录。

...
from output_layout import DIFF_TOOL, PERSON_VIEW, PROCESS_TOOL
...
class ContributionRepository:
    ...
    def _relative_source_path(lab: str, spec: _SourceSpec) -> str:
        return (Path(lab) / spec.directory / spec.filename(lab)).as_posix()
    ...
    def discover_tasks(
        ...
        selected = (
            [
                (item, self._safe_child(self.cleaned_root, PERSON_VIEW, item.directory_name))
                for item in self._all_students()
            ]

...
DEFAULT_CLEANED_ROOT = CLEANING_DIRECTORY.parent / "操作系统实验数据记录-已清洗"
REPOSITORY_ROOT = CLEANING_DIRECTORY.parent


def _display_path(path: Path | str) -> str:
    """Return repository-relative paths in machine-readable command output."""
    candidate = Path(path)
    try:
        return candidate.resolve(strict=False).relative_to(REPOSITORY_ROOT.resolve()).as_posix()
    except ValueError:
    ...
    *,
    ...
    result: dict[str, Any] = {
        **task_identity,
        ...
        "coverage": coverage,
        "message": message,
        "assessment": _display_path(storage.assessment_path(task_identity["student_directory"], task_identity["lab"])),
        "full_report": _display_path(storage.full_report_path(task_identity["student_directory"], task_identity["lab"])),
        "teacher_report": _display_path(storage.teacher_report_path(task_identity["student_directory"], task_identity["lab"])),
    }
    if run_log is not None:
        result["run_log"] = _display_path(run_log)
    return result
...
def build_parser() -> argparse.ArgumentParser:
    ...
    migrate = commands.add_parser("migrate-legacy", help="离线迁移旧版 AI 结果至双分类目录，不调用模型")
...
def _batch(args: argparse.Namespace) -> int:
    ...
    if overview_paths is not None:
        summary["overview_markdown"] = _display_path(overview_paths[0])
        summary["overview_csv"] = _display_path(overview_paths[1])
    if batch_summary_path is not None:
        summary["batch_summary"] = _display_path(batch_summary_path)
    _json_print(summary)

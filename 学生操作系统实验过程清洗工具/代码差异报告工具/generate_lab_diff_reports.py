...
from output_layout import DIFF_TOOL, ensure_layout, mirror_path, unlink_pair, write_text_pair
...
def map_cleaned_students(students: list[Student], cleaned_root: Path) -> None:
    ...
    by_source = {
        ...
        for key in (source_identity(student.source), student.source_reference)
        if key is not None
    }
...
def compare_lab(student: Student, lab: str, reference_root: Path,
    ...
    if not reference_lab.is_dir() or is_link_like(reference_lab):
        return ReportResult(student, lab, "失败", f"基准实验目录不存在：{reference_root.name}/{lab}")
...
def render_student_report(result: ReportResult, reference_root: Path,
    ...
    lines = [f"# {result.lab} 源码差异报告", "",
             ...
             f"- 原始提交目录：`{student.source_reference}`",
             f"- 基准目录：`{reference_root.name}/{result.lab}`",
...
def _report_manifest_owned_by(manifest: object, student: Student) -> bool:
    return (
        ...
        and (
            manifest.get("source") == student.source_reference
            or source_identity(manifest.get("source")) == source_identity(student.source)
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    known_sources = {
        ...
        for key in (
            ...
            source_identity(student.source_reference),
        )
        if key is not None
    }

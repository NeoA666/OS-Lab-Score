...
依赖：python3 -m pip install paramiko
...
# 服务器配置。密码已按需求写入脚本；请勿将此文件公开或提交到公共仓库。
HOST = "117.72.85.22"
PORT = 22
USERNAME = "root"
PASSWORD = "Hjl20041019"

REMOTE_ROOT = "/root/打包文件夹/xv6-submissions"
PROJECT_ROOT = Path(__file__).resolve().parent
OUTPUT_ROOT = PROJECT_ROOT / "操作系统实验数据清洗" / "操作系统实验数据记录"
...
def parse_index_row(row: dict[str, Any], date_dir: str) -> Submission:
...
def read_remote_indexes(sftp: Any) -> tuple[dict[str, Submission], list[str]]:
    ...
    for entry in sorted(sftp.listdir_attr(REMOTE_ROOT), key=lambda item: item.filename):
        ...
        for line_number, line in enumerate(text.splitlines(), 1):
            ...
            try:
                ...
                submission = parse_index_row(row, entry.filename)
...
def connect() -> tuple[Any, Any]:
    try:
        ...
        raise SyncError(
            "缺少 paramiko，请先运行：python3 -m pip install paramiko"
        ) from exc
...
def main() -> int:
    ...
    try:
        latest, warnings = read_remote_indexes(sftp)
